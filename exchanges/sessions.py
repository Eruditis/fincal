"""
Contains the functions GetSessions , GetActiveSession 
to parse session data from yml files

Usage:

# to run as a script
python fincal/exchanges/sessions.py
...
def GetSessions(date_: datetime, filepath: str):
    ...
    if not data_provided_from_date <= date_ <= data_provided_through_date:
        logging.info(f"date {date_} lies outside of data range in yaml files")
        ...
        sessions.append(
            {
                ...
                "session": data["intra_day_interruptions"][date_.date()]["session"],
                "gte": {
    ...
    if date__ in get_partial_trading_days(data):
        ...
        for partial_trading_hours in [
            x for x in data.keys() if re.search("^standard_partial.*_hours$", x)
        ]:
    ...
    if date_.strftime("%A") not in data["regular_trading_days"]:
        logging.info(f"date {date_} does not fall on a regular trading day")
        return []
...
def main():
    ...
    # date outside date range in yaml files
    pp.pprint(
        GetSessions(
            pd.Timestamp("2023-10-31", tz="America/New_York"), "exchanges/nyse.yml"
        )
    )
